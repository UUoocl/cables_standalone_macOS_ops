...
function getAddonPath() {
    const relative = "ops/Ops.Extension.Standalone.MacOs.Hid.ContourShuttlePro/contour_shuttle_pro.node";
...
function initAddon() {
    ...
    if (!fs.existsSync(addonPath)) {
        ...
        op.logError("[MacOs.Hid.ContourShuttlePro] Native addon not found at: " + addonPath);
        return false;
    }
    ...
    try {
        ...
        op.logError("[MacOs.Hid.ContourShuttlePro] Failed to load native addon: " + e.message);
        return false;
    }
}
...
function handleAddonEvent(jsonStr) {
    ...
    try {
        ...
        op.logWarn("[MacOs.Hid.ContourShuttlePro] Error parsing event payload: " + e.message);
    }
}
...
function ensureInitialized() {
    ...
    try {
        ...
        op.logError("[MacOs.Hid.ContourShuttlePro] Failed to start native controller: " + e.message);
        return false;
    }
}
