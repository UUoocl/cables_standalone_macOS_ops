 ...
 * 
 * Captures a WebGL Texture and spans/stretches it across the 3x2 (60x60 each = 180x120)
 * display matrix of a Soomfon controller device on macOS.
 */
...
const
    ...
    inFlipTileX = op.inBool("Flip Tile X", false),
...
inRender.onTriggered = () => {
        ...
        for (let c = 0; c < cols; c++) {
            ...
            if (tCanvas && tCtx) {
                ...
                if (inFlipTileX.get()) {
                    ...
                    tCtx.translate(kw, 0);
                    tCtx.scale(-1, 1);
                    tCtx.drawImage(canvasGrid, c * kw, r * kh, kw, kh, 0, 0, kw, kh);
                    tCtx.restore();
                } else {
                    tCtx.drawImage(canvasGrid, c * kw, r * kh, kw, kh, 0, 0, kw, kh);
                }
    ...
    Promise.all(uploadPromises)
        .catch((e) => {
            op.logWarn("[MacOs.Hid.SoomfonStretchedTexture] Upload error: " + e.message);
        })
        .finally(() => {
